...
class modOverlappingModel : Model
{
    List<byte[]> patterns;
    List<int> colors;
    ...
    public XElement GenerateXML(Dictionary<int, HashSet<int>> pixel_associations, Dictionary<int, String> tileColorMapping)
    {
        ...
        //okay... so we have a bit of a problem as there are two different ways we can do this. We have the name of the tiles being numbers... but 
        //doesn't that mean we can get the tile name somehow?
    ...
    public modOverlappingModel(string filepath, string name, int N, int width, int height, bool periodicInput, bool periodic, int symmetry, bool ground, Heuristic heuristic)
    ...
    public void SaveRules(string filename, List<Tile> tiles)
    {
        ...
        neighbors.Save(filename);
        Console.WriteLine("=============we are in overlapping, saving the neighbors===================");
