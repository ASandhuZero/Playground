...
An image showing the garbled pixel noise generated by WFC, this could be in the pixel bitmap example figure.
...
NOTE: Give a hypothesis
For testing, we used the Summer tile set and rule set that exists in the WFC repo, as that was the most tilemap like of the examples within WFC.

## how we evaluate the rules through the similarity test

To test if our approach generated rules were good approximants of crafted rules, we devised a straight forward test for rule set similarity.
First an image is generated with the Simpletiled model, using the Summer rule set, then this image is used as input to the Overlapping model.
The rules are then extracted during the Overlapping model's execution.
We then compare the generated rule set to the original rule set that was used to generate the first image.
For every rule that exists in both rule sets, an integer variable is increased for the purpose of keeping a count of all matching pairs.
One the rules have been compared, the total matching count is then divided by the total author crafted rules, giving a "coverage" percentage. 
Coverage, in our case, is the percentage of the *original* rules that exist within the generated set. 
...
As of now, when testing the Summer rule set by generating six 40x40 tilemaps, our approach managed a 100% rule coverage rate, but also generated an additional 90 rules. 
The original rule set contained 49 rules while the generated rule set would often reach around 150.

We believe that the additional rules are inferred rules that were generated by the Simpletiled model, which we explained in the technical description.
...
As an aside, in our testing, WFC began to fail around the 40x40 tilemap size, which corroborates Merrel's results.

We begin this is due to some tilemaps being more constrained than others.

A noticeable issue we were having was that even though WFC should collapse the entire wave, we were still having cells that had not been changed.

Our best guess is that since WFC does not modify the generative space like Model Synthesis, larger images will fail
...
We believe that this is due to the pattern association that is done with the Overlapping model and that without having a stronger way to state associations, 
After our testing, there should be a stronger pattern association process.
